...
PKGs="clang-18 lldb-18 lld-18 clangd-18 clang-tidy-18 clang-format-18 clang-tools-18 llvm-18-dev lld-18 lldb-18 llvm-18-tools libomp-18-dev libc++-18-dev libc++abi-18-dev libclang-common-18-dev libclang-18-dev libclang-cpp18-dev libunwind-18-dev libclang-rt-18-dev libpolly-18-dev"
PKGs="$(echo $PKGs | tr ' ' '\n' | sort -u)"
DEPS=""
for pkg in $PKGs; do
    LOCAL_DEPS="$(sudo apt-cache depends $pkg | grep Depends: | sed -E 's/Depends://g' | tr -d ' ')"
    DEPS="$DEPS $LOCAL_DEPS"
done
echo "$DEPS" | wc -c
DEPS="$(echo "$DEPS" | sort -u | tr ' ' '\n' | grep -v -E 'binutils|zlib1g|^python3$')"
echo "$DEPS" | wc -c

FILES=""
for dep in $DEPS; do
    LOCAL_FILES="$(dpkg -L $dep)"
    FILES="$FILES $LOCAL_FILES"
done
echo "$FILES" | wc -c
FILES="$(echo "$FILES" | sort -u | tr ' ' '\n')"
echo "$FILES" | tr ' ' '\n' | grep -v -E "/usr/share/clang|/usr/lib/llvm|/usr/lib/clang|/usr/lib/cmake/clang|/usr/include/llvm|/usr/bin/lldb|/usr/bin/llvm|/usr/bin/.*clang" | less
